...
def launch_subagent_cli(title, reason, hold_secs=0):
    script = os.path.join(os.path.dirname(__file__), "subagent_launcher.py")
    args = ["python3", script, title, "-r", reason]
    if hold_secs:
        args += ["--hold-secs", str(int(hold_secs))]
    p = subprocess.run(args, capture_output=True, text=True)
    out = {"ok": p.returncode==0, "returncode": p.returncode, "stdout": p.stdout.strip(), "stderr": p.stderr.strip()}
    try:
...
def launch_subagent(title, reason, hold_secs=0):
