...
def inject(batch):
    ...
    for _ in range(batch):
        ...
        obj={"ts":nowz(),"type":"action","subtype":"initiative","title":f"{topic} #{random.randint(1000,9999)}","content":"auto","priority":round(random.uniform(0.5,0.9),2),"topic":topic,"source":"initiative_daemon"}
        append(ART_THOUGHTS,obj); n+=1
    return n
def loop(base_sleep,batch):
    ensure()
    while True:
        mood=read_mood()
        fac=initiative_factors(mood)
        curr_sleep=max(30,int(base_sleep*fac["period_secs_mult"]))
        curr_batch=max(1,int(fac["batch"]))
        count=inject(max(curr_batch,batch))
        append(ART_LOG,{"ts":nowz(),"event":"injected","count":count,"batch":max(curr_batch,batch),"params":{"sleep":curr_sleep,"base_sleep":base_sleep,"mood":mood,"factors":fac}})
        time.sleep(curr_sleep)
def main():
    ap=argparse.ArgumentParser()
    ap.add_argument("--sleep",type=int,default=180)
    ap.add_argument("--batch",type=int,default=1)
    a=ap.parse_args()
    loop(a.sleep,a.batch)
